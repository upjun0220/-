...
DANGER_ZONES = {
    # [7/9] 케이블을 천장에 고정 -> FOV 안에 케이블 없음. 중앙 작업구역(신호 좋은 r<=1m)을 감시.
    #   레이더 유효 커버리지: 나디르(바로 밑) 기준 반경 ~1m. 그 밖은 천장레이더 특성상 반사가 약함.
    #   실제 설비 위치가 정해지면 그 (x,z) 사각으로 좁히면 됨.
    'A': {'x': (-1.0, 1.0), 'z': (-1.0, 1.0), 'label': 'WORK-ZONE'},
}
# [7/9] 케이블 천장 고정으로 FOV 내 오염원 제거됨 -> 공간 배제 불필요(빈 리스트 = 비활성).
#   혹시 나중에 특정 반사원을 통째로 배제하고 싶으면 {'x':(a,b),'z':(c,d)} 를 추가.
EXCLUDE_REGIONS = []
# [7/10] 근거리 아티팩트 게이트: 센서에서 이 거리(m) 이내의 점은 사람일 수 없음(천장형이라
#   센서 20~40cm 앞은 빈 공기). = 안테나 근거리/DC 누설 아티팩트 -> 파이프라인 초입에서 드롭.
#   실측: 이걸로 중앙 고스트가 사라져 off-nadir 사람 centroid가 제대로 나옴. 0 이면 비활성.
...
STAT_POS_R    = 0.8    # [7/10] 1.5->0.8: 빈방 잔여클러터가 1.5m 앵커 안에서 흩어져(실측 cx -0.81~0.92)
                       # 타이머를 채워 정지형 오탐. off-nadir 사람은 점 밀도 높아 centroid 안정 -> 0.8로 조임.
                       # (앵커: 정지 위치 고정 반경. 이보다 튀는 프레임=미스 -> hit_ratio 하락 -> 오탐 차단)
STAT_HIT_RATIO = 0.75  # [7/10] 0.65->0.75: 빈방 잔여클러터가 hit_ratio 0.62~0.64로 아슬아슬 발화.
                       #   실제 정지 사람(off-nadir 밀집)은 ~1.0이라 여유. 이건 '발화' 문턱.
STAT_HIT_FLOOR = 0.60  # [7/10] 신규: 충분히 관찰(stat_tot>=STAT_MIN_OBS) 후 hit_ratio가 이 값 미만이면
                       #   '흩어진 클러터=사람 아님'으로 보고 타이머 리셋(눌러앉기 방지).
STAT_MIN_OBS   = 20    # [7/10] 바닥 리셋 판단에 필요한 최소 관측 프레임(약 2초).
STAT_HIT_TIMEOUT = 2.0 # [7/12] 5->2초: 벽시계 dwell이 움직임을 견디며 자라 '이동 중 엉뚱한 앵커에서
                       #   PRE-ALERT' 오발화. 히트 없이 2초면 즉시 리셋 -> 연속 정지만 시간 축적.
...
STEP_IN_SEC     = 5.0    # [7/10] 빈방 스캔 후 사람이 들어와 자리(off-nadir) 잡을 카운트다운(초)
...
STAT_PRE_SEC  = 10.0   # [7/12] 15->10: 데모 시 15s 완전정지 유지가 어려워(실측 dwell 최대 6.9s에서
                       #   미세이동으로 리셋). 빈방 오탐 방어는 hit_ratio(0.75)·entry gate가 담당하므로
                       #   시간만 줄여도 안전(클러터 hit_ratio 0.64<0.75라 시간 무관하게 발화 불가).
STAT_CRIT_SEC = 30.0   # 2차: 계속 무동작 -> stationary 경보(critical, latch)
MAINT_MODE    = False  # True = 계획 정비 중(LOTO/작업허가) -> 정지형 경보 억제
STAT_MISS_TOL = 3      # [7/12] 10->3: 이탈 프레임 10개 용인이 '이동 중 타이머 생존 -> 오발화'의 주원인.
                       #   3개(~0.6s) 연속 이탈이면 즉시 리셋. 진짜 정지는 연속 히트라 안전, 이동은 바로 끊김.
                       #   (구 주석: 희소포인트 튐 용인 목적이었으나 그 대가로 이동 오발화가 커서 되돌림)
...
DETAILED_SOP_POPUP = False  # [7/12] True->False: 젯슨에서 gemma2:2b가 90s 타임아웃(사실상 미작동)이면서
                            #   메인 렌더 루프에 1.5->7s 스파이크를 박아 버튼(RESET) 반응이 몇 초씩 밀림(=먹통).
                            #   끄면 2번째 창/Gemma 호출 제거 -> 반응성 회복. 핵심 RAG(매뉴얼 검색)는 메인
                            #   패널에 그대로 표시됨. Gemma 정상화 후 다시 True.
...
LOAD_BASELINE = False  # [7/12] True->False: 옛 baseline 자동로드가 '빈방 클러터 스캔'을 건너뛰어
                       #   현재 방의 정지 반사체(고정위치 hit_ratio 1.0 유령)를 못 가림 -> 빈방 PRE-ALERT/오탐.
                       #   False면 매 실행 START 시 빈방 스캔+베이스라인을 새로 -> 현재 방에 맞는 클러터 마스킹.
                       #   (데모 안정 우선. 매번 ~40s 셋업 비용은 감수. 재사용 원하면 True + 방/레이더 고정)
...
FALL_CONFIRM   = 1       # [7/12] 2->1: 실측상 진짜 낙상(0.5s 사건)은 AE 3프레임 게이트 때문에
                         #   fall 판정이 창 안에 1번만 나와 2회 요구가 진짜 낙상을 버렸음(로그 확인:
                         #   h_drop 0.595/horiz 0.968/broad 4로 완벽 판정됐으나 미확정). 오탐 방어는
                         #   엄격한 shape 게이트 + POSTFALL_HOLD(1.2s 회복관찰)가 이미 담당 -> 1로 충분.
FALL_WIN_SEC   = 1.0     # [7/10] '연속' 대신 '최근 이 시간(초) 내 FALL_CONFIRM번'으로 확정.
                         #   impulse가 문턱 근처서 진동해 fall->normal->fall이 되면 연속이 깨져 진짜
                         #   낙상을 놓쳤음(16:25 실측). 짧은 창 카운트로 잡되 팔흔들기(1회)는 배제.
...
POSTFALL_GATE  = False   # [7/12] True->False: 라이브 로그상 이 게이트가 진짜 낙상을 즉시 취소함.
                         #   낙상 직후 낙하 잔여신호(n>=15, ds 0.3~0.9)가 회복(RECOVER) 조건과 겹쳐
                         #   1.2s 내 '일어남'으로 오판 -> latch 전에 삭제. 오탐 방어는 이미 shape 게이트
                         #   (horiz>=0.6·h_drop>=0.43·broad>=2·impulse)+RF veto가 담당(로그상 오탐 0건).
                         #   -> 낙상 확정 즉시 latch. 회복 취소 재설계 후 다시 켜려면 True.
...
def pipeline_loop():
    ...
    fall_hits   = []          # [7/10] 최근 fall_detected 시각들 (창 카운트 확정용)
    ...
    step_in_until = None      # [7/10] 입장 카운트다운 종료 시각 (None = 카운트다운 아님)
    read_offset = 0
    model       = None
    scaler      = None
    thr         = 0.01
    ...
    while True:
        ...
        for frame_pts in new_frames:
            # [7/10] 근거리 아티팩트 게이트: 센서 0.5m 이내 점(DC/근거리 누설, 사람 불가) 드롭
            if NEAR_FIELD_MIN_RANGE and frame_pts:
                frame_pts = [p for p in frame_pts if p['y'] >= NEAR_FIELD_MIN_RANGE]
            # [7/9] 케이블 오염구역 공간 배제 (흔들리는 케이블은 3D클러터로 못 잡음 -> 아예 드롭)
                ...
                if scan_until is not None:
                    if time.time() < scan_until:
                        ...
                        with _lock:
                            ...
                            state['sc_h'].append(0.0)
                        continue
                    _finish_scan()   # 스캔 종료 -> 클러터 맵 확정
                    step_in_until = time.time() + STEP_IN_SEC   # [7/10] 입장 카운트다운 시작

                # [7/10] STEP B-0: 입장 카운트다운 (사람이 들어와 off-nadir 자리 잡을 시간)
                if step_in_until is not None:
                    if time.time() < step_in_until:
                        with _lock:
                            state['scan_left'] = step_in_until - time.time()   # 진행바=카운트다운
                            state['sc_h'].append(0.0)
                        continue
                    step_in_until = None
                    with _lock:
                        state['scan_left'] = None
                    add_log('>> Collecting baseline NOW -- act NORMALLY (stand + small natural moves, do NOT freeze)')
            ...
            # [7/10] 안전장치: LIVE 확정 전(스캔/웜업/학습 중)엔 검출·경보를 절대 안 돌림.
            #   model이 어떤 이유로 안 지워져도 phase로 이중 차단 -> baseline 중 오탐/렉 방지.
            with _lock:
            ...
            # [7/10] hit_ratio 바닥 리셋: 흩어진 잔여클러터는 hit/miss를 반복해 '연속미스(MISS_TOL)'와
            #   '무히트 타임아웃'을 둘 다 피하며 hit_ratio ~0.5~0.6에 눌러앉아 타이머를 채움(빈방 오탐).
            #   -> 충분히 관찰(STAT_MIN_OBS) 후 hit_ratio가 floor 미만이면 '사람 아님'으로 보고 리셋.
            ...
            with _lock:
                ...
                if anom_streak >= CONFIRM_FRAMES:
                    ...
                    if et == 'normal':
                        pend_et, pend_cnt = None, 0   # 정상 -> non-fall 후보 리셋 (fall_hits는 시간감쇠라 안 지움)
                    elif et == 'fall_detected':
                        # [7/10] 낙상: '연속' 대신 '최근 FALL_WIN_SEC 내 FALL_CONFIRM번'으로 확정.
                        #   impulse 진동으로 fall->normal->fall이 돼도 창 안에 2번이면 잡힘.
                        _now = time.time()
                        fall_hits = [t for t in fall_hits if _now - t <= FALL_WIN_SEC]
                        fall_hits.append(_now)
                        if len(fall_hits) < FALL_CONFIRM:
                            et = 'normal'             # 아직 확정 아님 -> 보류
                        else:
                            fall_hits = []
                            pend_et, pend_cnt = None, 0
                    else:
                        # 나머지(진동/정지형) = CONFIRM_EVENTS 연속 동일 판정이어야 latch
