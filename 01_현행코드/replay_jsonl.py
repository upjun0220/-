...
#   실측 확인(2026-08-01): events_still.jsonl 에 y=0.033 (센서 3 cm) 같은 점이
#   섞여 있고, 이걸 그대로 흘리면 노트북의 머리 추정이 '높이 2.25 m'(천장 위!)
#   로 잡히고 누운 사람이 'standing' 으로 나온다.
#   → 재생기는 젯슨 자리를 대신하므로 젯슨과 같은 전처리를 태워야 한다.
#     (노트북 PoseEstimator 에 필터를 넣는 건 틀린 해법이다. 그건 젯슨 일이다)
