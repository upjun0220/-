...
# ── Static clutter removal (ceiling-mount cable etc.) ──
CLUTTER_DOPPLER_THR   = 0.05     # |doppler| below this -> "not moving"
CLUTTER_VOXEL_SIZE    = 0.10     # m, spatial bin size used to build the clutter map
CLUTTER_LEARN_FRAMES  = 100      # frames used at startup to learn fixed-clutter voxels
CLUTTER_OCCUPANCY_THR = 0.80     # a voxel must be "static" in >=80% of learn frames
CLUTTER_MANUAL_ZONES  = [
    # (x_min, x_max, y_min, y_max, z_min, z_max) in meters, radar-relative.
    # If you already know roughly where the cable sits (e.g. right above/behind
    # the radar near the ceiling mount), hardcode a box here for instant removal:
    # (-0.15, 0.15, -0.10, 0.10, 2.0, 2.5),
]
...
class StaticClutterFilter:
    """
    천장 고정 레이더에 연결된 케이블처럼, 항상 같은 위치에서 항상 속도가
    0에 가까운 포인트를 정적 클러터로 판정해 제거한다.

    동작 방식:
      1) '정지 후보' 판정: |doppler| < doppler_thr 인 포인트를 정지 상태로 표시
         (사람은 가만히 서 있어도 호흡/미세 움직임 때문에 doppler가 완전히
         0이 되는 경우가 드묾 -> 케이블/구조물과 구분 가능)
      2) 시작 후 learn_frames 프레임 동안, '정지 후보'가 매번 같은 복셀
         (voxel, 3D 격자 칸)에 반복 등장하는지 누적 관찰
      3) 관찰 종료 후 점유율이 occ_thr 이상인 복셀을 '고정 클러터'로 확정하여
         이후 계속 제거 (사람은 돌아다니므로 한 복셀 점유율이 낮게 유지됨)
      4) manual_zones 로 직접 지정한 박스는 학습 여부와 상관없이 항상 제거
         (케이블 위치를 이미 알고 있으면 여기 넣는 게 가장 빠름)
    """
    def __init__(self, doppler_thr=CLUTTER_DOPPLER_THR, voxel=CLUTTER_VOXEL_SIZE,
                 learn_frames=CLUTTER_LEARN_FRAMES, occ_thr=CLUTTER_OCCUPANCY_THR,
                 manual_zones=None):
        self.doppler_thr  = doppler_thr
        self.voxel        = voxel
        ...
        self._voxel_hits     = {}
        self._frame_count    = 0
        self._clutter_voxels = set()
        self._built = False

    def _key(self, p):
        return (round(p['x'] / self.voxel), round(p['y'] / self.voxel), round(p['z'] / self.voxel))

    def _in_manual_zone(self, p):
        for xmin, xmax, ymin, ymax, zmin, zmax in self.manual_zones:
            if xmin <= p['x'] <= xmax and ymin <= p['y'] <= ymax and zmin <= p['z'] <= zmax:
                return True
        return False

    def process(self, frame_pts):
        """학습(초반 learn_frames 동안) + 실시간 필터링을 함께 수행하고
        클러터가 제거된 포인트 리스트를 반환한다."""
        if not self._built:
            self._frame_count += 1
            for p in frame_pts:
                if abs(p.get('doppler', 0.0)) < self.doppler_thr:
                    k = self._key(p)
                    self._voxel_hits[k] = self._voxel_hits.get(k, 0) + 1
            if self._frame_count >= self.learn_frames:
                thr_cnt = self.occ_thr * self._frame_count
                self._clutter_voxels = {k for k, c in self._voxel_hits.items() if c >= thr_cnt}
                self._built = True
                add_log(f'Static clutter map built: {len(self._clutter_voxels)} voxel(s) '
                        f'identified as fixed clutter (cable/mount)')

        out = []
        for p in frame_pts:
            if self._in_manual_zone(p):
                continue
            if self._built and abs(p.get('doppler', 0.0)) < self.doppler_thr \
                    and self._key(p) in self._clutter_voxels:
                continue
            out.append(p)
        return out
...
def pipeline_loop():
    lms             = LMSFilter()
    clutter_filter  = StaticClutterFilter(manual_zones=CLUTTER_MANUAL_ZONES)
    feat_buf    = []
    warmup_feat = []
    prev_c      = None
    read_offset = 0
    ...
    add_log(f'Static clutter filter active (doppler_thr={CLUTTER_DOPPLER_THR}, '
            f'learning fixed clutter over {CLUTTER_LEARN_FRAMES} frames)')
    ...
    while True:
        ...
        for frame_pts in new_frames:
            ...
            # ── Static clutter removal (ceiling-mount cable etc.) ──
            frame_pts = clutter_filter.process(frame_pts)
