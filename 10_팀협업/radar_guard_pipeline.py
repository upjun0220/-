...
[수정 이력]
  v2.0 (2026-05-19) — Jetson Nano 이식을 위한 모델 자동 저장 추가
    - _train_model_impl() 완료 시 .pt 파일 자동 저장
    - 저장 위치: 이 파일과 같은 폴더 (radar_guard_model_{scenario}.pt)
    - 클로드한테 물어봐!.py (하드웨어 버전) 에서 해당 파일 로드
=================================================================
"""
...
# 훈련된 모델 저장 경로
# 클로드한테 물어봐!.py (하드웨어 버전) 와 반드시 동일한 경로를 사용하세요.
MODEL_SAVE_DIR = os.path.dirname(os.path.abspath(__file__))

# CWRU Bearing 데이터셋 상수 (진동이상 시나리오 전용)
...
def _train_model_impl(scenario="fall"):
    """
    내부 학습 로직 (Streamlit 의존 없음).
    진동이상 시나리오: CWRU 정상 베어링 데이터로 학습
    ...
    # ----------------------------------------------------------------
    # Jetson 이식용 모델 자동 저장
    # 저장 위치 (PowerShell에서 확인):
    #   {MODEL_SAVE_DIR}\radar_guard_model_{scenario}.pt
    # 클로드한테 물어봐!.py 가 이 파일을 로드해 실시간 추론에 사용합니다.
    # ----------------------------------------------------------------
    _save_model_for_jetson(model, scaler, threshold, scenario)

    return model, scaler, threshold


def _save_model_for_jetson(model, scaler, threshold, scenario):
    """
    훈련 완료된 모델·스케일러·임계치를 .pt 파일로 저장.
    클로드한테 물어봐!.py 의 load_trained_model() 과 포맷을 맞춥니다.

    저장 위치 (PowerShell):
        {MODEL_SAVE_DIR}\\radar_guard_model_{scenario}.pt
    """
    os.makedirs(MODEL_SAVE_DIR, exist_ok=True)
    path = os.path.join(MODEL_SAVE_DIR, f"radar_guard_model_{scenario}.pt")
    torch.save({
    ...
    # Streamlit 환경에서는 st.toast 로 알림, 일반 실행 시 print
    try:
        import streamlit as _st
        _st.toast(f"💾 모델 저장 완료: radar_guard_model_{scenario}.pt", icon="✅")
    except Exception:
        print(f"💾 모델 저장 완료: {path}")


@st.cache_resource(show_spinner=False)
def get_trained_model(scenario="fall"):
    """정상 패턴 학습 — 시나리오별 최초 1회만 실행, 이후 캐시 사용
    진동이상(vibration): CWRU 실제 베어링 데이터 사용 / 나머지: 목데이터
    """
    return _train_model_impl(scenario)
...
def seongjun_detect(stage1):
    ...
    model, scaler, threshold = get_trained_model(scenario)
...
if run_btn:
    ...
    # 모델 사전 학습 (시나리오별 최초 1회)
    cwru_note = " [CWRU 실제 베어링 데이터]" if scenario == "vibration" else " [목데이터]"
    with st.spinner(f"🧠 LSTM-AE 정상 패턴 학습 중...{cwru_note} (시나리오별 최초 1회만 실행)"):
        get_trained_model(scenario)
