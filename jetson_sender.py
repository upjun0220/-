...
try:
    ...
    # [8/19] 학습(train_fall_safety.py 161행)의 n_jobs=-1 이 모델에 절여져 추론까지
    #   따라온다. 샘플 1개에 트리 300개를 joblib 태스크로 흩뿌리고 공유배열에 락으로
    #   누적하므로 디스패치 비용이 연산을 압도한다. 실측 127.45ms -> 56.00ms.
    #   ⚠ 계산 방식만 바뀐다 — 같은 입력에 같은 출력이 나온다.
...
try:
    ...
    # [8/19] 위와 같은 이유. 이 모델은 매 프레임 호출돼 밀림의 주원인이었다
    #   (실측 rf=944.6ms/8프레임 = 118ms/frame, 전체 129.6ms 중 91%).
...
def pipeline_loop():
    ...
    while True:
        ...
        for _fnum, frame_pts in new_frames:
            ...
            # [8/19] RF 를 락 밖으로 옮겼다. _rf30_fall_score/_rf_fall_score 는
            #   state 를 읽지도 쓰지도 않고 지역 버퍼(clf_buf/rf30_buf)만 본다.
            #   락 안에 두면 프레임당 ~120ms 동안 10Hz sender_loop 이 패킷을 만들지
            #   못해 UDP 송신까지 함께 끊긴다. 판정값은 바뀌지 않는다.
