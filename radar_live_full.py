...
def classify(feat_win, score, thr):
    ...
    실측으로 확정한 판별치 (검증 정확도 49/50):
      - FALL        : 프레임 dop_std 피크 >= 1.2  (낙상 격렬함; 다른 동작 전부 <=1.16)
      - WALK(양성)  : n_mean >= 18               (보행 시 포인트 폭증 24~38)
      - 정지형 이상 : dop_std<0.6, 7<=n_mean<18, h_drop<1.1  (협착/감전 - 서로 구분 불가)
      - 그 외       : 빠른앉기/정상 등 양성 동작 -> 경고성 진동 이상
    ...
    dopstd_max  = max(float(f[4]) for f in win)
    dopstd_mean = sum(float(f[4]) for f in win) / len(win)   # 지속적 움직임 세기(순간 스파이크에 둔감)
    n_mean      = sum(float(f[6]) for f in win) / len(win)
    ...
    # 2) 정지형 이상 (협착/감전 통합) -- 제자리 + 낮은 도플러 + 중간 포인트.
    if dopstd_max < 0.6 and 8 <= n_mean < 18 and h_drop < 1.1:
        return {'event_type': 'stationary_anomaly', 'severity': 'critical', 'confidence': conf}

    # 3) 보행 / 정상 활동 -> 경보 없음
    if n_mean >= 18:
        return {'event_type': 'normal', 'severity': 'normal', 'confidence': 0.0}

    # 4) 지속적 동요만 경고 -- '평균' 도플러로 판단(순간 스파이크는 무시).
    #    실측상 정상 0.19~0.31 / 빠른앉기 0.25~0.39 < 0.40 -> 작은 움직임은 안 뜸.
    if dopstd_mean >= 0.40:
        ...
        for frame_pts in new_frames:
            ...
            with _lock:
                ...
                if anom_streak >= CONFIRM_FRAMES:
                    ...
                    et  = clf['event_type']
                    if et == 'normal':
                        pass   # 빈 공간/정지/정상/보행 -> 경보 만들지 않음 (오탐 방지)
                    else:
