...
STAT_POS_R    = 0.8    # [7/3 2차] 정지 위치 앵커 반경(m): 정지한 사람의 프레임별 centroid
                       # 편차는 최대 0.67m(실측 600프레임, p97<=0.44). 빈 방 노이즈는 방 전체를
                       # 점프(>1m) -> 앵커 밖 프레임은 히트로 안 침
STAT_HIT_RATIO = 0.65  # 타이머 진행 중 조건충족 프레임 비율 하한 (노이즈 ~50%는 못 채움)
STAT_HIT_TIMEOUT = 3.0 # 신선한 히트 없이 이 시간(초) 지나면 타이머 리셋 (사람 부재 정리)
...
CLUTTER_SPOT_R  = 0.30   # 학습 스팟 반경 (m)
...
def classify(feat_win, score, thr):
    ...
    if dopstd_max >= 1.2 and n_mean >= 5 and ds_broad >= 2:
