"""
Radar-Guard 통합 관제 시스템 v2-PC (week5-2_pc.py)
=====================================================
week5-2.py (하얀 배경 UI) + 찐최종 파이프라인 (Point Cloud 버전) 통합본.

변경 사항:
  - 기존 make_signal / extract_signal_features → make_point_cloud / extract_pc_features (PC 기반)
  - LSTM-AE 학습 + 이상 탐지 → 파이프라인 그대로 통합 (@st.cache_resource 캐싱)
  - 신호 분석 섹션 → Point Cloud 시각화 (재구성 오차 / Centroid Z / Doppler Std)
  - 3D Point Cloud → 실제 파이프라인 프레임 데이터 사용
  - UI 스타일/레이아웃은 week5-2.py 그대로 유지 (흰 배경)

실행 (PowerShell):
    streamlit run week5-2_pc.py

필수 사전 조건:
    docker start radar-guard-db
    Ollama 실행 중 (qwen2.5:3b-instruct-q4_K_M, bge-m3)
    pip install torch scikit-learn streamlit plotly langchain-ollama langchain-community
"""
...
# ===================================================================
st.set_page_config(
    page_title="Radar-Guard 관제 시스템 v2-PC",
    page_icon="🛡️",
    layout="wide",
...
with hcol1:
    st.title("🛡️ Radar-Guard 관제 시스템 v2-PC")
