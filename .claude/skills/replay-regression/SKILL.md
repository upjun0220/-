...
### replay_jsonl.py 옵션
```powershell
python replay_jsonl.py --list                 # 어느 파일에 뭐가 몇 건 있는지 먼저 확인
python replay_jsonl.py --seq fall,still,normal
python replay_jsonl.py --file ../03_데이터/이벤트_학습용/events_final.jsonl
python replay_jsonl.py --fast                 # 2배속
python replay_jsonl.py --once                 # 한 바퀴만
```
